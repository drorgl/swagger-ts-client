...
export class Operation implements IOperation {
    ...
    public build() {
        ...
        if (this.swOpr.parameters && this.swOpr.parameters.length) {
            this.operationParams = this.swOpr.parameters.map((p) => this.buildParam(p as Swagger.Parameter));
        }
    ...
    public getResponse(): string {
        if (this.swOpr.responses && this.swOpr.responses["200"] && (this.swOpr.responses["200"] as Swagger.Response).schema) {
            const retType = this.typeManager.getTypeNameInfo((this.swOpr.responses["200"] as Swagger.Response).schema);
    ...
    public buildParam(param: Swagger.Parameter): IOperationParam {
        ...
        if (!swParam) {
            swParam = (this.swOpr.parameters) ? (this.swOpr.parameters as Swagger.Parameter[]).find((v) => v.name == param.name) : null;
        }
        ...
        if (swParam) {
            swParam.name = changeCaseHelper(param.name, "camelCase");
        }

        return {
            paramName,
            paramDisplayName: changeCaseHelper(swParam.name, "camelCase"),
