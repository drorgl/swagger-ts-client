...
import { ISwaggerInlineType3 } from "./types";
...
export class RangeElectricResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available electric range resource for
     * the vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestRangeElectricUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType3>{
        return new Promise<ISwaggerInlineType3>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/rangeelectric`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType3);
                }
            });
        });
    }
