...
import { ISwaggerInlineType9 } from "./types";
...
export class DoorLockDeckLidStatusResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available door lock deck lid status
     * resource for the provided vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestDoorLockStatusdeckLidUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType9>{
        return new Promise<ISwaggerInlineType9>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/doorlockstatusdecklid`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType9);
                }
            });
        });
    }
