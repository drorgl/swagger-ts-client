...
import { ISwaggerInlineType11 } from "./types";
...
export class PositionHeadingResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available position heading resource for
    ...
    public getLatestPositionHeadingUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType11>{
        return new Promise<ISwaggerInlineType11>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/positionHeading`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType11);
                }
            });
        });
    }
