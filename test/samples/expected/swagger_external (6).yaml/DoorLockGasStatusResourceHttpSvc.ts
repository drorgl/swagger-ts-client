...
import { ISwaggerInlineType10 } from "./types";
...
export class DoorLockGasStatusResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available door lock status gas resource
     * for the provided vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestDoorLockStatusGasUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType10>{
        return new Promise<ISwaggerInlineType10>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/doorlockstatusgas`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType10);
                }
            });
        });
    }
