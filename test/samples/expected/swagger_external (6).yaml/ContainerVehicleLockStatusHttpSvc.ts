...
import { IVehicleLockStatus } from "./types";
...
export class ContainerVehicleLockStatusHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns all resources for the container
     * 'vehiclelockstatus', data can be filtered for a certain
     * time range.
     * @param { string } vehicleId Vehicle identification number
     */
    public getResourcesForContainerIdUsingGET(
         vehicleId:string ):Promise<Array<IVehicleLockStatus>>{
        return new Promise<Array<IVehicleLockStatus>>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/containers/vehiclelockstatus`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as Array<IVehicleLockStatus>);
                }
            });
        });
    }
