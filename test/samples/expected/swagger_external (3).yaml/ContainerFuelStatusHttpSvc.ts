...
export class ContainerFuelStatusHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns all resources for the provided 'fuelstatus', data
    ...
    public getResourcesForContainerIdUsingGET(
         vehicleId:string ):Promise<Array<IFuelStatus>>{
        return new Promise<Array<IFuelStatus>>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/containers/fuelstatus`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as Array<IFuelStatus>);
                }
            });
        });
    }
