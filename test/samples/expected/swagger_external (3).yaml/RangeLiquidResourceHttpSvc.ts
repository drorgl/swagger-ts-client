...
import { ISwaggerInlineType5 } from "./types";
...
export class RangeLiquidResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available range liquid level resource
    ...
    public getLatestRangeliquidUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType5>{
        return new Promise<ISwaggerInlineType5>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/rangeliquid`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType5);
                }
            });
        });
    }
