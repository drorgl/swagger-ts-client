...
import { ISwaggerInlineType27 } from "./types";
...
export class RearRightWindowsResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the rear right
    ...
    public getLatestWindowStatusRearRightUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType27>{
        return new Promise<ISwaggerInlineType27>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/windowstatusrearright`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType27);
                }
            });
        });
    }
