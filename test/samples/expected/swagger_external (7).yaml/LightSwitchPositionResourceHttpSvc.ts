...
import { ISwaggerInlineType19 } from "./types";
...
export class LightSwitchPositionResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the light switch
     * position and vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestLightSwitchPositionUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType19>{
        return new Promise<ISwaggerInlineType19>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/lightswitchposition`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType19);
                }
            });
        });
    }
