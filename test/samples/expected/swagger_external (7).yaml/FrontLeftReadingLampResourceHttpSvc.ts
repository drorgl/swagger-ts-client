...
import { ISwaggerInlineType20 } from "./types";
...
export class FrontLeftReadingLampResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the front left
    ...
    public getLatestReadingLampFrontLeftUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType20>{
        return new Promise<ISwaggerInlineType20>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/readingLampFrontLeft`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType20);
                }
            });
        });
    }
