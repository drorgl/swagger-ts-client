...
import { ISwaggerInlineType18 } from "./types";
...
export class InteriorRearLightResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the interior rear
     * lights and vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestInteriorRearLightsUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType18>{
        return new Promise<ISwaggerInlineType18>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/interiorLightsRear`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType18);
                }
            });
        });
    }
