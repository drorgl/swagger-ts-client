...
import { IVehicleStatus } from "./types";
...
export class ContainerVehicleStatusHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns all resources for the provided 'vehiclestatus',
    ...
    public getResourcesForContainerIdUsingGET(
         vehicleId:string ):Promise<Array<IVehicleStatus>>{
        return new Promise<Array<IVehicleStatus>>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/containers/vehiclestatus`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as Array<IVehicleStatus>);
                }
            });
        });
    }
