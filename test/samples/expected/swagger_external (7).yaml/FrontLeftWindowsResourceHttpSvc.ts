...
import { ISwaggerInlineType24 } from "./types";
...
export class FrontLeftWindowsResourceHttpSvc {
    ...
    public getLatestWindowStatusFrontLeftUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType24>{
        return new Promise<ISwaggerInlineType24>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/windowstatusfrontleft`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType24);
                }
            });
        });
    }
