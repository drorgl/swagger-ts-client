...
import { ISwaggerInlineType26 } from "./types";
...
export class RearLeftWindowsResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the rear left
    ...
    public getLatestWindowStatusRearLeftUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType26>{
        return new Promise<ISwaggerInlineType26>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/windowstatusrearleft`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType26);
                }
            });
        });
    }
