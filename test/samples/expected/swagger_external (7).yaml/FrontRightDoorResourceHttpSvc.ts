...
import { ISwaggerInlineType14 } from "./types";
...
export class FrontRightDoorResourceHttpSvc {
     ...
     * door status and vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestDoorStatusFrontRightUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType14>{
        return new Promise<ISwaggerInlineType14>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/doorstatusfrontright`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType14);
                }
            });
        });
    }
