...
import { ISwaggerInlineType16 } from "./types";
...
export class RearRightDoorResourceHttpSvc {
    ...
    public getLatestDoorStatusRearRightUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType16>{
        return new Promise<ISwaggerInlineType16>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/doorstatusrearright`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType16);
                }
            });
        });
    }
