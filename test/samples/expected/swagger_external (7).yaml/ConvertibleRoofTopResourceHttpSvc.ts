...
import { ISwaggerInlineType22 } from "./types";
...
export class ConvertibleRoofTopResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the convertible
    ...
    public getLatestRoofTopStatusUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType22>{
        return new Promise<ISwaggerInlineType22>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/rooftopstatus`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType22);
                }
            });
        });
    }
