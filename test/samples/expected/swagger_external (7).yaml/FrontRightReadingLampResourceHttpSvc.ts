...
import { ISwaggerInlineType21 } from "./types";
...
export class FrontRightReadingLampResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the front right
     * reading lamp and vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestReadingLampFrontRightUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType21>{
        return new Promise<ISwaggerInlineType21>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/readingLampFrontRight`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType21);
                }
            });
        });
    }
