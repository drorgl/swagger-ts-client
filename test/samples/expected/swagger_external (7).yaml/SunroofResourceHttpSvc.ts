...
import { ISwaggerInlineType23 } from "./types";
...
export class SunroofResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the sunroof and
     * vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestSunRoofStatusUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType23>{
        return new Promise<ISwaggerInlineType23>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/sunroofstatus`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType23);
                }
            });
        });
    }
