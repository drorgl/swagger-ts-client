...
import { ISwaggerInlineType13 } from "./types";
...
export class FrontLeftDoorResourceHttpSvc {
    ...
    public getLatestDoorStatusFrontLeftUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType13>{
        return new Promise<ISwaggerInlineType13>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/doorstatusfrontleft`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType13);
                }
            });
        });
    }
