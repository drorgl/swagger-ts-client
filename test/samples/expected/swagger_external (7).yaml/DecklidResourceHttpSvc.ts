...
import { ISwaggerInlineType12 } from "./types";
...
export class DecklidResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the decklid
     * status and vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestDecklidUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType12>{
        return new Promise<ISwaggerInlineType12>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/decklidstatus`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType12);
                }
            });
        });
    }
