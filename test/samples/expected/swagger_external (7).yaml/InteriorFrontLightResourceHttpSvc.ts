...
import { ISwaggerInlineType17 } from "./types";
...
export class InteriorFrontLightResourceHttpSvc {
    constructor(private baseUrl:string  = "https://api.arik-benz.com/vehicledata_tryout/v1"){

    }

    /**
     * Returns the latest available resource for the interior
     * front lights and vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestInteriorFrontLightsUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType17>{
        return new Promise<ISwaggerInlineType17>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/interiorLightsFront`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType17);
                }
            });
        });
    }
