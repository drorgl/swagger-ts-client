...
import { ISwaggerInlineType25 } from "./types";
...
export class FrontRightWindowsResourceHttpSvc {
     ...
     * window status and vehicle identification number.
     * @param { string } vehicleId Vehicle identification number
     */
    public getLatestWindowStatusFrontRightUsingGET(
         vehicleId:string ):Promise<ISwaggerInlineType25>{
        return new Promise<ISwaggerInlineType25>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/vehicles/${encodeURIComponent(vehicleId)}/resources/windowstatusfrontright`)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as ISwaggerInlineType25);
                }
            });
        });
    }
