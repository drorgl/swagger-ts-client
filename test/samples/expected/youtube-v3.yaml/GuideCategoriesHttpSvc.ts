...
import { IGuideCategoryListResponse } from "./types";
...
export class GuideCategoriesHttpSvc {
     ...
     * YouTube channels.
     * @param { string } hl The hl parameter specifies the language that will be used
     *        for text values in the API response.
     * @param { string } id The id parameter specifies a comma-separated list of the
     *        YouTube channel category ID(s) for the resource(s) that are
     *        being retrieved. In a guideCategory resource, the id
     *        property specifies the YouTube channel category ID.
     ...
     *        list of guide categories available in the specified
     ...
     */
    public youtube_guideCategories_list(
         hl:string ,
         id:string ,
         part:string ,
         regionCode:string ,
         ...
         prettyPrint:boolean ,
         quotaUser:string ,
         userIp:string ):Promise<IGuideCategoryListResponse>{
        const params = {
            "hl":hl,
            "id":id,
            "part":part,
        ...
        return new Promise<IGuideCategoryListResponse>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/guideCategories`)
            .query(params)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as IGuideCategoryListResponse);
                }
            });
        });
    }
