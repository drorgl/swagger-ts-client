...
import { II18nRegionListResponse } from "./types";
...
export class I18nRegionsHttpSvc {
    constructor(private baseUrl:string  = "https://www.googleapis.com/youtube/v3"){

    }

    /**
     * Returns a list of content regions that the YouTube website
     * supports.
     ...
     */
    public youtube_i18nRegions_list(
         hl:string ,
         part:string ,
         alt:string ,
         fields:string ,
         ...
         prettyPrint:boolean ,
         quotaUser:string ,
         userIp:string ):Promise<II18nRegionListResponse>{
        const params = {
            "hl":hl,
            "part":part,
            "alt":alt,
        ...
        return new Promise<II18nRegionListResponse>((resolve, reject) => {
            superAgentRequest
            .get(this.baseUrl + `/i18nRegions`)
            .query(params)
            .end(function(error, res){
                if(error){
                    reject(error);
                }else{
                   resolve(res.body as II18nRegionListResponse);
                }
            });
        });
    }
